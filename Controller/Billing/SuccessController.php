...
class SuccessController extends Controller
{
    ...
    public function billingPaymentSuccess(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($subscriptionId = $form->get('subscriptionId')->getData()) {
                ...
                $this->sendNewCardConfirmationEmail($tenant);
            }
     ...
     * @param Tenant $tenant
     */
    private function sendNewCardConfirmationEmail(Tenant $tenant)
    {
        try {
            $client = new PostmarkClient($this->getParameter('annex.postmark_api_key'));
            $message = $this->renderView('AnnexTenantBundle::emails/new_card.html.twig',
                ['tenant' => $tenant]
            );
            $client->sendEmail(
                "Annex Apps <system@annex-apps.com>",
                $tenant->getOwnerEmail(),
                "Thanks for adding a new card",
                $message
            );
